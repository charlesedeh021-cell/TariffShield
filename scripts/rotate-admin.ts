 ...
 *  6. Write the new secret to a local file (restricted permissions) and print
 *     update instructions that reference the file path instead of embedding
 *     the raw secret in console output.
 *
 * Flags:
 ...
 *   --show-secret   Print the raw secret key to stdout (use only in secure,
 *                   non-logged terminals; never use in CI).
 *
 * Usage:
 *   npx ts-node scripts/rotate-admin.ts [--dry-run] [--new-secret SXXX...] [--show-secret]
 *
 * SECURITY NOTE: Do not add --show-secret to any CI/CD pipeline invocation.
 * This script may run in automated contexts where stdout is captured in logs.
 */
...
export function parseArgs(): { dryRun: boolean; newSecret: string | null; showSecret: boolean } {
  ...
  return { dryRun, newSecret, showSecret };
}

/** Mask a Stellar secret key for safe display: show class prefix + first 4 chars then asterisks. */
function maskSecret(secret: string): string {
  // Stellar secrets start with "S" and are 56 chars. Show "S" + next 3 + asterisks.
  if (secret.length < 5) return "****";
  return `${secret.slice(0, 4)}${"*".repeat(secret.length - 4)}`;
}

/**
 * Write the new admin secret to a local file with restrictive permissions (0600).
 * Returns the absolute path of the written file.
 *
 * SECURITY NOTE: Do not log secrets to stdout — this script may run in automated
 * contexts where stdout is captured in logs. Write to a file instead and reference
 * the file path in any printed instructions.
 */
function writeSecretToFile(secret: string, publicKey: string): string {
  const filename = `tariffshield-new-admin-${Date.now()}.key`;
  const filepath = path.join(os.tmpdir(), filename);
  const content = [
    "# TariffShield new admin secret — generated by rotate-admin.ts",
    `# Public key: ${publicKey}`,
    `# Generated at: ${new Date().toISOString()}`,
    "# Delete this file after updating all deployment environments.",
    "",
    secret,
    "",
  ].join("\n");
...
async function main() {
  const { dryRun, newSecret: providedNewSecret, showSecret } = parseArgs();
  ...
  // Write the secret to a restricted-permission local file instead of printing
  // it to stdout. This script may run in CI or a logged terminal session where
  // stdout is captured — never embed raw secrets in console output by default.
  const secretFilePath = writeSecretToFile(newAdmin.secret(), newAdmin.publicKey());
...
============================================================
  ...
  New admin secret key  : ${maskSecret(newAdmin.secret())} (masked)
  Secret written to     : ${secretFilePath}  (mode 0600)

  ⚠️  IMPORTANT — read the secret from the file above and
      update PLATFORM_STELLAR_SECRET in your deployment
      environment, then redeploy the API before revoking
      the old secret. Delete the key file afterwards.

  Render (manual):
    Dashboard → tariffshield-api → Environment → PLATFORM_STELLAR_SECRET
    Set to the value in: ${secretFilePath}

  Vercel (manual):
    Dashboard → tariffshield-web → Settings → Environment Variables
    Update PLATFORM_STELLAR_SECRET to the value in: ${secretFilePath}

  AWS / other:
    SECRET=$(cat ${secretFilePath} | grep -v '^#' | tr -d '\\n') && \\
...
`);
  ...
  if (showSecret) {
    console.warn("⚠️  --show-secret flag is set. Raw secret (do not copy into CI logs):");
    console.log(`  New admin secret key: ${newAdmin.secret()}`);
  }
}
