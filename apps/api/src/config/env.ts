...
export const Env = z.object({
  PORT: z.coerce
    ...
    .positive()
    .default(3002)
    .describe('Port for the API server to listen on'),
  NODE_ENV: z
  ...
  PG_POOL_MAX: z.coerce
    ...
    .positive()
    .default(20)
    .describe('Max concurrent PostgreSQL connections per API instance (issue #264, #241)'),
  PG_IDLE_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(30000)
    .describe('Milliseconds an idle PostgreSQL client is kept before being closed'),
  PG_CONN_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(3000)
    .describe('Milliseconds to wait for a new PostgreSQL connection before failing fast'),
  PG_STATEMENT_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(10000)
    .describe('Milliseconds before a single PostgreSQL statement is cancelled (issue #241)'),
  REDIS_URL: z
    .string()
  ...
  STELLAR_NETWORK: z
    .enum(['testnet', 'public'])
    .default('testnet')
    .describe('Stellar network to connect to'),
  ...
  TARIFF_SHIELD_CONTRACT_ID: z
    .string()
    .startsWith('C')
    .min(56)
    .describe('Soroban contract ID for TariffShield'),
  PLATFORM_STELLAR_SECRET: z
    .string()
    .startsWith('S')
    .min(56)
    .describe('Admin/platform Stellar secret key'),
  ADMIN_2_SECRET: z
    .string()
    .startsWith('S')
    .min(56)
    .optional()
    .describe('Admin 2 Stellar secret key for multi-sig upgrade'),
  ADMIN_3_SECRET: z
    .string()
    .startsWith('S')
    .min(56)
    .optional()
    .describe('Admin 3 Stellar secret key for multi-sig upgrade'),
  SURETY_STELLAR_SECRET: z
    .string()
    .startsWith('S')
    .min(56)
    .describe('Surety provider Stellar secret key'),
  METRICS_ALLOWED_CIDR: z
    .string()
    .optional()
    .describe('CIDR block allowed to access Prometheus metrics'),
  CBP_VALIDATION_MODE: z
    .enum(['warn', 'block'])
    .default('block')
    .describe('CBP lookup failure mode'),
  ORACLE_ALERT_THRESHOLD_PCT: z.coerce
    .number()
    .default(50)
  ...
  PRICE_ORACLE_CONTRACT_ID: z
    ...
    .min(56)
    .optional()
    .describe('Soroban contract ID for the USDC/USD price oracle'),
  EMERGENCY_ADMIN_SECRET: z
    .string()
    .startsWith('S')
    .min(56)
    .optional()
    .describe('Emergency oracle admin Stellar secret key'),

  // #339 — separate oracle role keypair
  ORACLE_STELLAR_SECRET: z
    .string()
    .startsWith('S')
    .min(56)
    .optional()
    .describe('Oracle-role Stellar secret key (separate from PLATFORM_STELLAR_SECRET)'),

  // #308 — SAML 2.0 SSO
  SAML_SP_ENTITY_ID: z
    .string()
    .optional()
    .describe('SAML Service Provider entity ID (e.g. https://tariffshield.io/saml/metadata)'),
  SAML_SP_ACS_URL: z
    .string()
    .url()
    .optional()
    .describe('SAML Assertion Consumer Service (ACS) callback URL'),
  ...
  SAML_AZURE_CERT: z
    .string()
    .optional()
    .describe('Azure AD IdP X.509 certificate (PEM, no headers)'),
  ...
  DOCUSIGN_WEBHOOK_HMAC_KEY: z
    .string()
    .optional()
    .describe('HMAC key for verifying DocuSign Connect webhook signatures'),
  ...
  FIELD_ENCRYPTION_KEY: z
    ...
    .optional()
    .describe(
      'AES-256-GCM key for encrypting EIN and PII fields (min 32 chars; use KMS-derived key in prod)'
    ),
  FIELD_ENCRYPTION_KEY_VERSION: z.coerce
    .number()
    .int()
    .positive()
    .default(1)
  ...
  S3_KYC_BUCKET: z
    .string()
    .optional()
    .describe('S3 bucket name for KYC document storage (SSE-KMS encrypted)'),
  ...
  SENDGRID_API_KEY: z
    .string()
    .optional()
    .describe('SendGrid API key for compliance report email notifications'),
  REPORT_FROM_EMAIL: z
    .string()
    .email()
    .optional()
