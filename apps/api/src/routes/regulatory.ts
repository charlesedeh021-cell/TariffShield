...
regulatoryRouter.get('/state-report/:state_code', async (req: Request, res: Response) => {
  ...
  const importerSegmentation = segmentQuery.rows.map(
    (row: { business_state: string; importer_count: string }) => ({
      businessState: row.business_state,
      count: parseInt(row.importer_count, 10),
    })
  );

  const totalImporterCount = importerSegmentation.reduce(
  ...
  if (format === 'csv') {
    let csvData =
      'State Code,Total Bonds Written,Aggregate Face Value,Claims Filed,Collateral Held On-Chain,Importer Business Registration State,Importer Count\r\n';
