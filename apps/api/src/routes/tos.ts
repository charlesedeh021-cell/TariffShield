...
tosRouter.post('/accept-tos', async (req: Request, res: Response) => {
  ...
  const version = await pool.query('SELECT version_id FROM tos_versions WHERE version_id = $1', [
  ...
  await pool.query('UPDATE users SET tos_reacceptance_required = FALSE WHERE id = $1', [user.id]);
